...
#include <filesystem>
	...
	#define HZ_INTERNAL_VERIFY_WITH_MSG(type, check, ...) HZ_INTERNAL_VERIFY_IMPL(type, check, "Verify failed: {0}", __VA_ARGS__)
	#define HZ_INTERNAL_VERIFY_NO_MSG(type, check) HZ_INTERNAL_VERIFY_IMPL(type, check, "Verify '{0}' failed at {1}:{2}", HZ_STRINGIFY_MACRO(check), std::filesystem::path(__FILE__).filename().string(), __LINE__)
